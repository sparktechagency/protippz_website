...
const WithDrawPage = () => {
    ...
    const handleSubmit = async (values: any) => {
        try {
            ...
            if (amount > data?.userData?.totalAmount) {
            ...
            const res = await post('/withdraw/create', paymentMethod == 'ach' ? ach : check, {
                headers: {
        ...
        <div style={{ width: '100vw' }} className='w-full container mx-auto'>
            <div className="w-full p-8 bg-white rounded-lg max-w-4xl mx-auto">
                ...
                <h2 className="text-3xl font-semibold text-center mb-6 text-blue-900">
                    <span className='text-[#2FC191]'>Total Funds : </span> ${data?.userData?.totalAmount}
                </h2>
                    ...
                    <div className="w-full md:w-3/5 p-4 bg-gray-50 rounded-lg">
                        {paymentMethod === 'ach' ? (
                            <Form layout='vertical' onFinish={handleSubmit}>
                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Bank Account Number</span>}
                                    name="bankAccountNumber"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your bank account number!' }]}
                                >
                                    <Input placeholder="Enter Bank Account Number" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Routing Number</span>}
                                    name="routingNumber"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your routing number!' }]}
                                >
                                    <Input placeholder="Enter Routing Number" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Account Type</span>}
                                    name="accountType"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your account type!' }]}
                                >
                                    <Input placeholder="Enter Account Type" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Bank Name</span>}
                                    name="bankName"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your bank name!' }]}
                                >
                                    <Input placeholder="Enter Bank Name" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Account Holder Name</span>}
                                    name="accountHolderName"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your account holder name!' }]}
                                >
                                    <Input placeholder="Enter Account Holder Name" className="rounded-lg border border-green-400" />
                                </Form.Item>
                            ...
                            <Form layout='vertical' onFinish={handleSubmit}>
                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Player/Team Name</span>}
                                    name="name"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your Player/Team Name!' }]}
                                >
                                    <Input placeholder="Enter Player/Team Name" className="rounded-lg border border-green-400" />
                                </Form.Item>
                                <p className='text-xl'>Mailing Address</p>
                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Email</span>}
                                    name="email"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your Email!' }]}
                                >
                                    <Input type='email' placeholder="Enter Email" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Street Address</span>}
                                    name="streetAddress"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your street address!' }]}
                                >
                                    <Input placeholder="Enter Street Address" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">City</span>}
                                    name="city"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your city!' }]}
                                >
                                    <Input placeholder="Enter City" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">State</span>}
                                    name="state"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your state!' }]}
                                >
                                    <Input placeholder="Enter State" className="rounded-lg border border-green-400" />
                                </Form.Item>

                                <Form.Item
                                    label={<span className="text-lg font-medium text-blue-900">Zip Code</span>}
                                    name="zipCode"
                                    className="mt-4"
                                    rules={[{ required: true, message: 'Please input your zip code!' }]}
                                >
                                    <Input placeholder="Enter Zip Code" className="rounded-lg border border-green-400" />
                                </Form.Item>
