...
//                     </> : <>
//                         <p><strong>Enter Your Shipping Address Carefully:</strong> For physical prizes like gear or merchandise, please provide your full name and accurate mailing address in the designated fields. Double-check your street address, city, state, and zip code to ensure there are no errors. Incorrect or incomplete information may cause delivery delays or issues.</p>
//                         <p className='my-3'><strong>Check Your Shipping Details:</strong> Before submitting, review your address carefully. Make sure the details are complete and correct, as the shipment will be sent to the address provided. If any changes are needed, please update the information before submitting.</p>
//                         <p><strong>Confirmation:</strong> Once your address is submitted, your physical prizes will be shipped to the provided address. You will receive a confirmation message and tracking details to your email once the items are dispatched.</p>
...
const RedeemButton: React.FC<SendTipsButtonProps> = ({ item }) => {
    ...
    const [isEmailVerifiedModalOpen, setIsEmailVerifiedModalOpen] = useState(false);
    const [isShippingAddressModalOpen, setIsShippingAddressModalOpen] = useState(false);
    const [isEmailVerificationCodeModalOpen, setIsEmailVerificationCodeModalOpen] = useState(false);
    ...
    const handleGuideModalNext = () => {
        ...
        item?.category?.deliveryOption == 'Email' ? setIsEmailVerificationModalOpen(true) : setIsShippingAddressModalOpen(true)
    ...
    const showShippingAddressModalSubmit = async (value: any) => {
        const res = await post('/redeem-request/create', {
            "reward": item?._id,
    ...
    const onOtpInput = async (value: any) => {
        const res = await post(`/redeem-request/verify-redeem-email/${redeemId}`, { verifyCode: Number(value?.verifyCode) }, {
            headers: {
                "Authorization": `${localStorage.getItem('token')}`
            }
        })
    ...
    return (
        <>
            <button onClick={showGuideModal} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none">
                Redeem
            </button>
            {/* Guide Modal */}
            <Modal
                ...
                footer={[
                    <button onClick={handleGuideModalNext} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none">
                        Redeem
                    </button>
                ]}
            >
            ...
            <Modal
                ...
                footer={[
                    <Button key="verify" type="primary" onClick={handleEmailVerificationCodeModalOk}>
                        Verify Email
                    </Button>,
                ]}
            >
                <Form layout="vertical">
                    <Form.Item label="Email" name="email" rules={[{ required: true, message: 'Please enter your email' }]}>
                        <Input onChange={(e) => setEmail(e.target.value)} className='h-[42px]' placeholder="Your email" />
                    </Form.Item>
                </Form>
            </Modal>
            ...
            >
                <div className="flex flex-col items-center p-8 rounded-lg max-w-lg w-full bg-white">
                    ...
                    <Form onFinish={onOtpInput} name="" layout="vertical" className="w-full">
                        <Form.Item label="Please enter the 6 digit code" name="verifyCode" className='text-center flex justify-center items-center' rules={[{ required: true, message: 'Please enter your Otp' }]}>
                            <Input.OTP length={5} className="h-[42px]" />
                        </Form.Item>

                        <Form.Item>
                            <Button type="primary" htmlType="submit" className="w-full bg-[#053697] hover:bg-[#467eee] h-[42px] max-w-[320px] mx-auto block">
                                Verify Otp
                            </Button>
                        </Form.Item>
                    </Form>
            ...
            >
                <div className="flex justify-center items-center gap-3 flex-col py-10">
                    <Image src={check} alt='check' height={100} width={100} />
                    <p className="text-lg font-semibold mt-4">Request Send Successfully!</p>
                </div>
            </Modal>
            ...
            >
                <Form onFinish={showShippingAddressModalSubmit} layout="vertical">
                    <Form.Item label="Full Name" name={`userName`} rules={[
                        ...
                        <Input placeholder="Enter full name" className="rounded-lg border border-green-400" />
                    </Form.Item>
                    <Form.Item label="Phone Number" name={`phone`} rules={[
                        ...
                        <Input placeholder="Enter Phone Number" className="rounded-lg border border-green-400" />
                    </Form.Item>

                    <Form.Item label="Mailing Address" className="mt-4">
                        <Form.Item label="Street Address" name={`streetAddress`} rules={[
                            {
                                message: 'Street Address is required',
                                required: true
                            }
                        ]} className="mb-2">
                            <Input placeholder="Street Address" className="rounded-lg border border-green-400" />
                        </Form.Item>
                        <Form.Item label="City" name={`city`} rules={[
                            ...
                            <Input placeholder="City" className="rounded-lg border border-green-400" />
                        </Form.Item>
                        <Form.Item label="State" name={`state`} rules={[
                            ...
                            <Input placeholder="State" className="rounded-lg border border-green-400" />
                        </Form.Item>
                        <Form.Item label="Zip Code" name={`zipCode`} rules={[
                            ...
                            <Input placeholder="Zip Code" className="rounded-lg border border-green-400" />
                        </Form.Item>
                    </Form.Item>

                    <Form.Item className="mt-4">
                        <Button type="primary" htmlType="submit" className="w-full rounded-lg bg-blue-900 text-white">
                            Submit
                        </Button>
                    </Form.Item>
                </Form>
