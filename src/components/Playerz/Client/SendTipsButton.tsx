...
const SendTipsButton: React.FC<SendTipsButtonProps> = ({ item }) => {
    ...
    const handlePaymentSubmit = async (value: any) => {
        ...
        if (value?.paymentMethod == 'deposit') {
            ...
            router.push(`/send-tip?amount=${amount}&entityType=Player&entityId=${item?._id}`)
        }
        ...
        <>
            ...
            >
                <div className="text-center">
                    ...
                    <p><span className="text-green-500 font-semibold">Team:</span> {item.team?.name}</p>
                    <p><span className="text-green-500 font-semibold">Position:</span> {item.position}</p>
                </div>
                <Form form={form}
